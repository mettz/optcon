...
def nonlinear_system(variables, u):
    # Define your nonlinear system of equations with inputs
    x, y, psi, V, beta, psi_dot = variables
    delta, Fx = u
...
def find_equilibrium_point(u):
    # Initial random guess for the equilibrium point
    # initial_guess = np.random.randint(10, size=8)
    initial_guess = np.array([0, 0, 0, 1.3, 4, 0])
    print("Initial guess:", initial_guess)
    equilibrium_point = fsolve(nonlinear_system, initial_guess, args=(u,))
    return equilibrium_point


# Example input
u = [0.0, 0.0]
equilibrium_point = find_equilibrium_point(u)
